...
export function AlertHistoryWidget() {
  ...
  return (
    <div className="flex flex-col h-full">
      {data?.lastCheck && (
        <div className="px-3 py-1 border-b border-zinc-200 dark:border-zinc-800 text-xs text-zinc-400 dark:text-zinc-600 shrink-0">
      ...
      {!isLoading && alerts.length === 0 && (
        <div className="flex flex-col items-center justify-center h-full gap-1">
          ...
          <span className="text-xs text-zinc-400 dark:text-zinc-600">{data?.error ?? 'No alerts in last 7 days'}</span>
        </div>
      )}
          ...
          {alerts.map((alert, i) => (
            <div key={i} className="flex items-center gap-3 px-3 py-2">
              ...
              <div className="flex-1 min-w-0">
                <div className="text-xs text-zinc-800 dark:text-zinc-200 truncate">{alert.label}</div>
                <div className="text-xs text-zinc-400 dark:text-zinc-600">{formatTime(alert.timestamp)}</div>
              </div>
            </div>
          ))}
        </div>
