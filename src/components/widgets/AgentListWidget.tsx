    ...
    <div className="flex flex-col h-full">
      ...
      {!isLoading && agents.length === 0 && (
        <div className="flex flex-col items-center justify-center h-full gap-2 px-4 text-center">
          ...
          <p className="text-xs text-zinc-600">No agents connected</p>
          <p className="text-xs text-zinc-700">
            Run <code className="font-mono bg-zinc-800 px-1 rounded">clawd-monitor-agent --server ...</code> on your OpenClaw VPS
          </p>
        </div>
      )}
            ...
            return (
              <button
                ...
                className={`w-full text-left px-3 py-2.5 hover:bg-zinc-800/30 transition-colors ${
                  isActive ? 'bg-indigo-900/20 border-l-2 border-indigo-500' : ''
                }`}
              >
                ...
                {agent.lastSnapshot && (
                  <div className="flex gap-3 mt-0.5 text-xs text-zinc-600 ml-4">
                    <span>{agent.lastSnapshot.sessions?.length ?? 0} sessions</span>
                    <span>{agent.lastSnapshot.cronJobs?.length ?? 0} crons</span>
                    <span className="font-mono text-zinc-700">v{agent.version}</span>
                  </div>
                )}
              </button>
            )
