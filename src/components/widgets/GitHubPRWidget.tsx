    ...
    <div className="flex flex-col h-full">
      ...
      {!isLoading && !data?.error && prs.length === 0 && (
        <div className="flex flex-col items-center justify-center h-full gap-1">
          ...
          <span className="text-xs text-zinc-600">No open PRs</span>
        </div>
      )}
        ...
        <div className="overflow-y-auto divide-y divide-zinc-800/50 flex-1">
          {prs.map((pr) => (
            <a
              ...
              className="flex items-start gap-2 px-3 py-2 hover:bg-zinc-800/20 transition-colors block"
            >
              <div className={`w-1.5 h-1.5 rounded-full mt-1 shrink-0 ${CI_DOT[pr.ciStatus]}`} />
              <div className="flex-1 min-w-0">
                <div className="text-xs text-zinc-200 truncate">
                  {pr.draft && <span className="text-zinc-600 mr-1">[draft]</span>}
                  {pr.title}
                </div>
